...
-- beautiful init
beautiful.init(os.getenv("HOME") .. "/.config/awesome/themes/pheew/theme.lua")
...
-- {{{ Tags
tags = {
   names = { "term", "web", "docs", "media", "files", "other" },
   layout = { layouts[2], layouts[10], layouts[4], layouts[1], layouts[7], layouts[1] }
}
...
for s = 1, screen.count() do
	...
	for w = 1, #widgets.top.left do
		left_layout:add(mypromptbox[s])
		left_layout:add(widgets.top.left[w])
	end
	...
	for w = 1, #widgets.top.right do
		right_layout:add(widgets.top.right[w])
	end
